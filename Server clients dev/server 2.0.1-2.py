...
##Setting end
sde = False     # sde = Shut Down Enable
##Setting end
...
def shutdown():
    global chat
    chat.append("Console: Shutdown in 10 Seconds")
    time.sleep(5)
    chat.append("Console: Shutdown in 5 Seconds")
    time.sleep(1)
    chat.append("Console: Shutdown in 4 Seconds")
    time.sleep(1)
    chat.append("Console: Shutdown in 3 Seconds")
    time.sleep(1)
    chat.append("Console: Shutdown in 2 Seconds")
    time.sleep(1)
    chat.append("Console: Shutdown in 1 Seconds")
    time.sleep(0.1)
    os._exit(0)
...
def recv(conn):
    ...
    while True:
        ...
        if data == "admin -s":
            ...
            time.sleep(0.1)
            clear = False
        elif data[0:8] == "admin -t":
            result = data.split(":")[1].strip()
            print(result)
            result = str("Console: " + result)
            chat.append(result)
            print(result)
        elif data[0:9] == "admin -sd":
            if sde == True:
                threading.Thread(target=shutdown, args=()).start()
        else:
            if data != '':
                chat.append(data)
                print(chat)
